...
namespace Xten
{
    namespace orm
    {
        ...
        {
            return Xten::ToLower(m_name + m_subfix);
        }
        ...
        bool Table::init(const tinyxml2::XMLElement &node)
        {
            ...
            {
                XTEN_LOG_ERROR(g_logger) << "table name is null";
                return false;
            }
            ...
            {
                XTEN_LOG_ERROR(g_logger) << "table namespace is null";
                return false;
            }
            ...
            if (!cols)
            {
                XTEN_LOG_ERROR(g_logger) << "table name=" << m_name << " columns is null";
                return false;
            }
            ...
            if (!col)
            {
                XTEN_LOG_ERROR(g_logger) << "table name=" << m_name << " column is null";
                return false;
            }
            ...
            do
            {
                ...
                if (!col_ptr->init(*col))
                {
                    XTEN_LOG_ERROR(g_logger) << "table name=" << m_name << " init column error";
                    return false;
                }
                if (col_names.insert(col_ptr->getName()).second == false)
                {
                    XTEN_LOG_ERROR(g_logger) << "table name=" << m_name << " column name="
            ...
            if (!idxs)
            {
                XTEN_LOG_ERROR(g_logger) << "table name=" << m_name << " indexs is null";
                return false;
            }
            ...
            if (!idx)
            {
                XTEN_LOG_ERROR(g_logger) << "table name=" << m_name << " index is null";
                return false;
            }
            ...
            do
            {
                ...
                if (!idx_ptr->init(*idx))
                {
                    XTEN_LOG_ERROR(g_logger) << "table name=" << m_name << " index init error";
                    return false;
                }
                if (idx_names.insert(idx_ptr->getName()).second == false)
                {
                    XTEN_LOG_ERROR(g_logger) << "table name=" << m_name << " index name="
                ...
                if (idx_ptr->isPK())
                {
                    if (has_pk)
                    {
                        XTEN_LOG_ERROR(g_logger) << "table name=" << m_name << " more than one pk";
                        return false;
                    }
                    has_pk = true;
                }
                ...
                for (auto &x : cnames)
                {
                    ...
                    {
                        XTEN_LOG_ERROR(g_logger) << "table name=" << m_name << " idx="
        ...
        void Table::gen_inc(const std::string &path)
        {
            ...
            std::set<std::string> incs = {"Xten/include/db.h", "Xten/include/util.h"};
            for (auto &i : incs)
            {
                ofs << "#include \"" << i << "\"" << std::endl;
            }
            ...
            std::vector<std::string> ns = Xten::split(m_namespace, '.');
        ...
        std::string Table::genToStringSrc(const std::string &class_name)
        {
                 ...
                 it != m_cols.end(); ++it)
            {
                ...
                {
                    ss << "Xten::Time2Str(" << GetAsMemberName((*it)->getName()) << ")"
                       << ";" << std::endl;
                }
                else
                {
            ...
            ss << "    return Xten::JsonUtil::ToString(jvalue);" << std::endl;
            ss << "}" << std::endl;
            return ss.str();
        }
        ...
        void Table::gen_src(const std::string &path)
        {
            ...
            ofs << "#include \"Xten/include/log.h\"" << std::endl;
            ofs << std::endl;
            ...
            ofs << std::endl;
            ofs << "static Xten::Logger::ptr g_logger = XTEN_LOG_NAME(\"orm\");" << std::endl;
...
#define CHECK_STMT(v)                                                          \
    ...
    ofs << "        XTEN_LOG_ERROR(g_logger) << \"stmt=\" << sql" << std::endl \
        << "                 << \" errno=\""                                   \
