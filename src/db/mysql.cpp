...
namespace Xten
{
    ...
    static Xten::ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_mysql_dbs = Xten::Config::LookUp("mysql.dbs", std::map<std::string, std::map<std::string, std::string>>(), "mysql dbs");
                             ...
                             const int &timeout)
    {
        ...
        static thread_local MySQLThreadIniter s_thread_initer; // 为每个线程创建专属mysql客户端上下文信息

        MYSQL *mysql = ::mysql_init(nullptr);
        if (mysql == nullptr)
        {
            XTEN_LOG_ERROR(g_logger) << "mysql_init error";
            return nullptr;
        }
        ...
        int port = Xten::GetParamValue(params, "port", 0);
        std::string host = Xten::GetParamValue<std::string>(params, "host");
        std::string user = Xten::GetParamValue<std::string>(params, "user");
        std::string passwd = Xten::GetParamValue<std::string>(params, "passwd");
        std::string dbname = Xten::GetParamValue<std::string>(params, "dbname");

        if (mysql_real_connect(mysql, host.c_str(), user.c_str(), passwd.c_str(), dbname.c_str(), port, NULL, 0) == nullptr)
        {
            XTEN_LOG_ERROR(g_logger) << "mysql_real_connect(" << host
                                     << ", " << port << ", " << dbname
    ...
    bool MySQL::connect()
    {
        ...
        m_hasError = false;
        m_poolSize = Xten::GetParamValue(m_params, "pool", 5);
        m_mysql.reset(m, mysql_close);
        return true;
    }

    Xten::IStmt::ptr MySQL::prepare(const std::string &sql)
    {
    ...
    {
        m_cmd = Xten::StringUtil::Formatv(format, ap);
        int r = ::mysql_query(m_mysql.get(), m_cmd.c_str());
        if (r)
        {
            XTEN_LOG_ERROR(g_logger) << "cmd=" << cmd()
                                     << ", error: " << getErrStr();
            m_hasError = true;
        }
        else
    ...
    {
        return MySQL::ptr(this, Xten::nop<MySQL>);
    }
    ...
    static MYSQL_RES *my_mysql_query(MYSQL *mysql, const char *sql)
    {
        if (mysql == nullptr)
        {
            XTEN_LOG_ERROR(g_logger) << "mysql_query mysql is null";
            return nullptr;
        }

        if (sql == nullptr)
        {
            XTEN_LOG_ERROR(g_logger) << "mysql_query sql is null";
            return nullptr;
        }

        if (::mysql_query(mysql, sql))
        {
            XTEN_LOG_ERROR(g_logger) << "mysql_query(" << sql << ") error:"
                                     << mysql_error(mysql);
            return nullptr;
        }
        ...
        if (res == nullptr)
        {
            XTEN_LOG_ERROR(g_logger) << "mysql_store_result() error:"
                                     << mysql_error(mysql);
        }
        return res;
    }
    ...
    MySQLStmt::ptr MySQLStmt::Create(MySQL::ptr db, const std::string &stmt)
    {
        ...
        {
            XTEN_LOG_ERROR(g_logger) << "stmt=" << stmt
        ...
        MySQLStmt::ptr rt = Xten::protected_make_shared<MySQLStmt>(db, st);
...
#define BIND_COPY_LEN(ptr, size)                                \
    ...
    int MySQLStmt::bindTime(int idx, const time_t &value)
    {
        ...
        // return 0;
        return bindString(idx, Xten::Time2Str(value));
    }
    ...
    {
        return Xten::TypeUtil::Atoi(m_cur[idx]);
    }
    ...
    {
        return Xten::TypeUtil::Atof(m_cur[idx]);
    }
    ...
    time_t MySQLRes::getTime(int idx)
    {
        ...
        return Xten::Str2Time(m_cur[idx]);
    }
    ...
    MySQLStmtRes::ptr MySQLStmtRes::Create(std::shared_ptr<MySQLStmt> stmt)
    {
        ...
        MySQLStmtRes::ptr rt = Xten::protected_make_shared<MySQLStmtRes>(stmt, eno, errstr);
        if (eno)
        {
            return rt;
        }
        ...
        if (!res)
        {
            return Xten::protected_make_shared<MySQLStmtRes>(stmt, stmt->getErrno(), stmt->getErrStr());
        }
...
#undef XX
    ...
    ISQLData::ptr MySQL::query(const char *format, va_list ap)
    {
        ...
        {
            m_hasError = true;
            // XTEN_LOG_ERROR(g_logger) << "query: " << m_cmd << " fail, errno="
            //     << getErrno() << " errstr=" << getErrStr();
            return nullptr;
        }
        m_hasError = false;
        ISQLData::ptr rt = std::make_shared<MySQLRes>(res, mysql_errno(m_mysql.get()), mysql_error(m_mysql.get()));
        return rt;
    }
    ...
    {
        MySQLTransaction::ptr rt = Xten::protected_make_shared<MySQLTransaction>(mysql, auto_commit);
        if (rt->begin())
        {
            return rt;
        }
    ...
    int MySQLTransaction::execute(const char *format, va_list ap)
    {
        if (m_isFinished)
        {
            XTEN_LOG_ERROR(g_logger) << "transaction is finished, format=" << format;
            return -1;
        }
    ...
    int MySQLTransaction::execute(const std::string &sql)
    {
        if (m_isFinished)
        {
            XTEN_LOG_ERROR(g_logger) << "transaction is finished, sql=" << sql;
            return -1;
        }
    ...
    MySQL::ptr MySQLManager::get(const std::string &name)
    {
        ...
        if (!m_isStart)
        {
            // 启动定时器检查连接
            m_isStart = true;
            Xten::IOManager::GetThis()->addTimer(60 * 1000,
                                                 std::bind(&MySQLManager::checkConnection, this, 30), true);
        }
        ...
        if (it != m_conns.end())
        {
            if (!it->second.empty())
            {
                ...
                {
                    XTEN_LOG_WARN(g_logger) << "reconnect " << name << " fail";
                    delete rt;
                    return nullptr;
                }
            }
    ...
    void MySQLManager::checkConnection(int sec)
    {
        ...
        lock.unlock();
        XTEN_LOG_DEBUG(g_logger) << "CheckConnection, Expired Connection Count=" << conns.size();
        for (auto &i : conns)
        {
            delete i;
        }
    ...
    int MySQLManager::execute(const std::string &name, const char *format, va_list ap)
    {
        ...
        if (!conn)
        {
            XTEN_LOG_ERROR(g_logger) << "MySQLManager::execute, get(" << name
                                     << ") fail, format=" << format;
            return -1;
        }
    ...
    ISQLData::ptr MySQLManager::query(const std::string &name, const char *format, va_list ap)
    {
        ...
        if (!conn)
        {
            XTEN_LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name
                                     << ") fail, format=" << format;
            return nullptr;
        }
    ...
    MySQLTransaction::ptr MySQLManager::openTransaction(const std::string &name, bool auto_commit)
    {
        ...
        if (!conn)
        {
            XTEN_LOG_ERROR(g_logger) << "MySQLManager::openTransaction, get(" << name
                                     << ") fail";
            return nullptr;
        }
    ...
    {
        auto m = MySQLManager::GetInstance()->get(name);
        if (!m)
        {
            XTEN_LOG_ERROR(g_logger) << "mysql name=" << name << " not exists";
            return nullptr;
        }
