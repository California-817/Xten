...
static Xten::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_thread_info_set
...
void FoxThread::read_cb(evutil_socket_t sock, short which, void* args) {
    ...
    if(recv(sock, cmd, sizeof(cmd), 0) > 0) {
        ...
        lock.unlock();
        thread->m_working = true; //working
        for(auto it = callbacks.begin();
                it != callbacks.end(); ++it) {
            if(*it) {
                //XTEN_ASSERT(thread == GetThis());
                try {
                    (*it)(); //执行上层派发的函数
                } catch (std::exception& ex) {
                    XTEN_LOG_ERROR(g_logger) << "exception:" << ex.what();
                } catch (const char* c) {
                    XTEN_LOG_ERROR(g_logger) << "exception:" << c;
                } catch (...) {
                    XTEN_LOG_ERROR(g_logger) << "uncatch exception";
                }
            } else {
                //让内部进行event事件循环的线程终止event_base_dispatch函数，保证异常是快速终止线程
        ...
        //总共执行任务个数++
        Xten::Atomic::addFetch(thread->m_total, callbacks.size());
...
FoxThread::FoxThread(const std::string& name, struct event_base* base)
    ...
    //初始化读写的socketfd
    int fds[2];
    if(evutil_socketpair(AF_UNIX, SOCK_STREAM, 0, fds) == -1) { //创建匿名管道
        //XTEN_LOG_ERROR(g_logger) << "FoxThread init error";
...
void FoxThread::start() {
    if(m_thread) {
        //XTEN_LOG_ERROR(g_logger) << "FoxThread is running";
...
bool FoxThread::dispatch(callback cb) {
    ...
    //通过管道通知内部线程有任务需要处理，内部线程跳转到readCb中处理任务
...
void FoxThread::stop() {
    RWMutex::WriteLock lock(m_mutex);
    m_callbacks.push_back(nullptr); //派发一个空任务，内部线程在readCb中获知后调用event_base_loopbreak跳出事件循环
    if(m_thread) {
        uint8_t cmd = 0;
        //write(m_write, &cmd, sizeof(cmd));
...
bool FoxThreadPool::dispatch(callback cb) {
    do {
        Xten::Atomic::addFetch(m_total, (uint64_t)1);
        RWMutex::WriteLock lock(m_mutex);
        if(!m_advance) {
...
bool FoxThreadPool::batchDispatch(const std::vector<callback>& cbs) {
    Xten::Atomic::addFetch(m_total, cbs.size());
    RWMutex::WriteLock lock(m_mutex);
    if(!m_advance) {
        for(auto cb : cbs) {
...
//从空闲线程池中取得线程并分配队首任务，利用 shared_ptr + 自定义 deleter 自动归还线程并触发后续调度，从而实现“仅在线程空闲时才派发任务”的可靠策略。
...
const std::string& FoxThread::GetFoxThreadName() {
    ...
    uint64_t tid = Xten::ThreadUtil::GetThreadId();
    do {
...
void FoxThread::setThis() {
    m_name = m_name + "_" + std::to_string(Xten::ThreadUtil::GetThreadId());
    s_thread = this;

    RWMutex::WriteLock lock(s_thread_mutex);
    s_thread_names[Xten::ThreadUtil::GetThreadId()] = m_name;
}

void FoxThread::unsetThis() {
    s_thread = nullptr;
    RWMutex::WriteLock lock(s_thread_mutex);
    s_thread_names.erase(Xten::ThreadUtil::GetThreadId());
}
...
void FoxThreadManager::init() {
    ...
    for(auto i : m) {
        auto num = Xten::GetParamValue(i.second, "num", 0);
        auto name = i.first;
        auto advance = Xten::GetParamValue(i.second, "advance", 0);
        if(num <= 0) {
            XTEN_LOG_ERROR(g_logger) << "thread pool:" << name
                        << " num:" << num
                        << " advance:" << advance
                        << " invalid";
            continue;
        }
        if(num == 1) { //线程个数1，说明是一个线程
            m_threads[i.first] = std::make_shared<FoxThread>(i.first);
            XTEN_LOG_INFO(g_logger) << "init thread : " << i.first;
        } else {
            //线程个数大于1，说明是一个线程池
            m_threads[i.first] = std::make_shared<FoxThreadPool>(
                            num, name, advance);
            XTEN_LOG_INFO(g_logger) << "init thread pool:" << name
                       << " num:" << num
                       << " advance:" << advance;
        }
    }
...
void FoxThreadManager::start() {
    for(auto i : m_threads) {
        XTEN_LOG_INFO(g_logger) << "thread: " << i.first << " start begin";
        i.second->start();
        XTEN_LOG_INFO(g_logger) << "thread: " << i.first << " start end";
    }
}

void FoxThreadManager::stop() {
    for(auto i : m_threads) {
        XTEN_LOG_INFO(g_logger) << "thread: " << i.first << " stop begin";
        i.second->stop();
        XTEN_LOG_INFO(g_logger) << "thread: " << i.first << " stop end";
    }
    for(auto i : m_threads) {
        XTEN_LOG_INFO(g_logger) << "thread: " << i.first << " join begin";
        i.second->join();
        XTEN_LOG_INFO(g_logger) << "thread: " << i.first << " join end";
    }
}
