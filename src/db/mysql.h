...
private:
    ...
    std::shared_ptr<MYSQL_RES> m_data; //管理mysqlClient实现的结果集
...
public:
    ...
    //这里支持stmt预处理语句的执行(本质是用到了下面实现的MysqlStmt对象)
    Xten::IStmt::ptr prepare(const std::string& sql) override;
...
private:
    bool isNeedCheck();
private:
    std::map<std::string, std::string> m_params; //dbserver消息
    std::shared_ptr<MYSQL> m_mysql;

    std::string m_cmd;  //上次指令
    std::string m_dbname; //数据库name

    uint64_t m_lastUsedTime; //上次使用时间
    bool m_hasError; //是否又错误
    int32_t m_poolSize;
};
//Mysql数据库事务执行对象实现(内部含有一个Mysql连接对象)
...
//管理所有的mysql数据库连接对象----update
//单例管理所有mysql数据库连接对象
class MySQLManager : public singleton<MySQLManager>{
public:
    typedef Xten::FiberMutex MutexType; //优化：使用协程锁提高并发
    //并不会初始化连接
    MySQLManager();
    ~MySQLManager();
    //这个mysql连接池不是简单的静态连接池，而是通过get方法动态临时创建连接----非阻塞获取连接
    //外部使用的连接数量可以很多，但是池中最多只能存放每个name定义的maxConn数量的连接---回收的时候进行判断
    ...
    //开启事务返回一个支持事务的mysql连接对象
    MySQLTransaction::ptr openTransaction(const std::string& name, bool auto_commit);
private:
    void freeMySQL(const std::string& name, MySQL* m);
private:
    MutexType m_mutex; //锁
    std::map<std::string, std::list<MySQL*> > m_conns; //每个name对应一批mysql连接
    std::map<std::string, std::map<std::string, std::string> > m_dbDefines; //定义参数 registerMySQL方法临时注册
    bool m_isStart=false;
};
//这个类提供一些简化的静态方法操作mysql数据库
class MySQLUtil {
public:
