...
// 如果找到则将该Segment转移到rcv_queue中等待下次用户再调用recv接收数据 。
//
// 需要注意的是，Segment在从buf转到queue中时会确保转移的Segment的sn号为下次需要接收的，
// 否则将不做转移，rcv_queue 的数据是连续的--可靠性，rcv_buf 可能是间隔的
//
...
{
	struct IQUEUEHEAD *p;
	int ispeek = (len < 0)? 1 : 0; //用户是否peek
	int peeksize; //recv队列中将要接收的包大小
	int recover = 0; //是否进行窗口恢复
	IKCPSEG *seg;
	assert(kcp);

	if (iqueue_is_empty(&kcp->rcv_queue)) //接收队列空
		return -1;

	if (len < 0) len = -len; 

	peeksize = ikcp_peeksize(kcp); //获取包大小
	...
	for (len = 0, p = kcp->rcv_queue.next; p != &kcp->rcv_queue; ) {
		int fragment;//分片号---后面还有几个分片
		seg = iqueue_entry(p, IKCPSEG, node);
		p = p->next;

		if (buffer) {
			//拷贝一个fragment的用户数据到用户缓冲区
		...
		if (ispeek == 0) { 
			//不是peek而是正常接收逻辑，队列中去除这个包
			iqueue_del(&seg->node);
			ikcp_segment_delete(kcp, seg);
			kcp->nrcv_que--;
			//否则是peek，这个包不动，但是用户可以拿到
		}
	...
	while (! iqueue_is_empty(&kcp->rcv_buf)) {
		...
		if (seg->sn == kcp->rcv_nxt && kcp->nrcv_que < kcp->rcv_wnd) {
			// 接收缓冲区的这个包的sn就是想要的下一个包的sn----->保证顺序性和连续性 && 接收窗口还有空间
			iqueue_del(&seg->node);
			kcp->nrcv_buf--;
...
//---------------------------------------------------------------------
// peek data size 提前获知接收队列中将要接收的包数据大小
//---------------------------------------------------------------------
int ikcp_peeksize(const ikcpcb *kcp)
{
	...
	seg = iqueue_entry(kcp->rcv_queue.next, IKCPSEG, node);
	if (seg->frg == 0) return seg->len; //未分片
...
// - 包模式，数据分片赋予独立id，依次放入snd_queue，接收方按照id解分片数据，分片大小 <= mss
...
int ikcp_send(ikcpcb *kcp, const char *buffer, int len)
{
	...
	if (kcp->stream != 0) { //开启了流模式，每个segment数据大小都会尽可能的保证是一个mss大小
		if (!iqueue_is_empty(&kcp->snd_queue)) {
			IKCPSEG *old = iqueue_entry(kcp->snd_queue.prev, IKCPSEG, node);
			if (old->len < kcp->mss) { //填充最后一个segment
				//创建一个新segment，将原来最后一个segment的数据和当前要发送的数据组合，放入新segment 删除旧segment
...
static void ikcp_parse_ack(ikcpcb *kcp, IUINT32 sn)
{
	...
	for (p = kcp->snd_buf.next; p != &kcp->snd_buf; p = next) {
		IKCPSEG *seg = iqueue_entry(p, IKCPSEG, node);
		next = p->next;
		if (sn == seg->sn) { //收到响应后，删除这个响应的sn对应在发送缓冲区的包
			iqueue_del(p);
			ikcp_segment_delete(kcp, seg);
			kcp->nsnd_buf--;
			break;
		...
		//** 如果收到的是远端发来的数据包
			...
			if (_itimediff(sn, kcp->rcv_nxt + kcp->rcv_wnd) < 0) {
				...
				if (_itimediff(sn, kcp->rcv_nxt) >= 0) {
					...
					// 解析data, 
					// 首先会在rcv_buf中遍历一次，判断是否已经接收过这个数据包，
					// 如果数据包不存在则添加到rcv_buf中[有序放入]，之后将可用的Segment再转移到rcv_queue中
...
//---------------------------------------------------------------------
// ikcp_encode_seg   将一个结构化数据segment序列化成字节流数据
//---------------------------------------------------------------------
...
void ikcp_flush(ikcpcb *kcp)
{
	...
	// flush remain segments buffer数据中小于mtu大小的剩余数据进行发送
	size = (int)(ptr - buffer);
	if (size > 0) {
		ikcp_output(kcp, buffer, size);
		++kcp->snd_sum;
