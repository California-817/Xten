//=====================================================================
//
// KCP - A Better ARQ Protocol Implementation       ARQ（Automatic Repeat-reQuest，自动重传请求）
...
//		- KCP.Segment.frg frg是fragment的缩小，是一个Segment在一次Send的data中的倒序序号，可理解为该分片后续还有几个包。 
...
struct IKCPSEG
{
	...
	IUINT32 frg;      // Fragment 记录了分片时的倒序序号, 当输出数据大于 MSS 时，需要将数据进行分片；fragment碎片
	IUINT32 wnd;      // Window, 己方的可用窗口大小, 也就是 rcv_queue 的可用大小即 rcv_wnd - nrcv_que, 见 ikcp_wnd_unused 函数
	IUINT32 ts;       // Timestamp, 记录了发送时的时间戳，用来估计 RTT：RTT（Round-Trip Time，往返时延）从发送方发出 一段数据 开始，到 收到对应确认（ACK） 为止所经历的时间。
...
//	snd_buf 发送消息的缓存
//  snd_buf 如下图所示
//  snd_una 发送的所有请求中第一个没有收到响应的包
//	snd_nxt 下一个待分配的sn---->包从发送队列放到sendbuffer中
//	+---+---+---+---+---+---+---+---+---+---+---+---+---+
//	...	| 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9 | ...........
//	+---+---+---+---+---+---+---+---+---+---+---+---+---+
//		^					            ^				^
...
struct IKCPCB
{
	...
	//rcv_nxt：接收队列中需要的下一个segment的sn
	//snd_una：发送缓冲区第一个未确认的包
	//snd_nxt：发送缓冲区中下一个待分配的包的sn
	...
	IUINT32 *acklist; // ack列表，所有收到的包ack将放在这里，依次存放sn和ts [sn0 ts0 sn1 ts1 sn2 ts2 ..... ]
	IUINT32 ackcount; // ack数量
	IUINT32 ackblock; // acklist大小
	void *user;  //上层创建时传入的参数
	char *buffer; // 存储消息字节流的内存
	int fastresend; // 触发快速重传的重复ack个数
	int nocwnd, stream; // 非退让流控、流模式[没有接口进行设置 默认不开启]
	int logmask;
	int(*output)(const char *buf, int len, struct IKCPCB *kcp, void *user); // 底层网络传输函数
	void(*writelog)(const char *log, struct IKCPCB *kcp, void *user); //日志输出函数
};
...
//从kcpcb的接收队列获取一个segment中的数据----用户数据
// -len表示peek len表示正常读取 len表示buffer大小
int ikcp_recv(ikcpcb *kcp, char *buffer, int len);

// when you received a low level packet (eg. UDP packet), call it
//将用户层调用udp的recvfrom接口获得的原始数据放到kcpcb的接收缓冲区中
...
//将用户数据构造kcp的segment并放到kcpcb的发送队列
int ikcp_send(ikcpcb *kcp, const char *buffer, int len);

// update state (call it repeatedly, every 10ms-100ms), or you can ask 
// ikcp_check when to call it again (without ikcp_input/_send calling).
// 'current' - current timestamp in millisec. 
//外部需要循环调用这个接口，因为这个接口会周期性调用flush将数据发送出去
//内部flush间隔时间默认为100ms
...
//set internal flush data time default:100ms
...
// nodelay: 0:disable(default), 1:enable  是否非延迟
// interval: internal update timer interval in millisec, default is 100ms  内部刷新数据间隔时间
// resend: 0:disable fast resend(default), 1:enable fast resend 快速重传次数
// nc: 0:normal congestion control(default), 1:disable congestion control 取消拥塞控制
